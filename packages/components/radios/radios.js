...
export default () => {
  ...
  const radioInputs = document.querySelectorAll('.govuk-radios--conditional .govuk-radios__input')

  /**
   * Update all conditional reveals to match checked state
   */
  const syncAllConditionalReveals = () => {
    radioInputs.forEach((input) => toggleConditionalInput(input, 'govuk-radios__conditional--hidden'))
  }
